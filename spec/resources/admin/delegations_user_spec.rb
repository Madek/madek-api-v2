...
def delegation_url(user_id, delegation_id)
  "/api-v2/admin/delegations/users/#{user_id}/delegations/#{delegation_id}"
end
...
RSpec.describe "Admin::Delegations API", type: :request do
  ...
  let(:base_url) { "/api-v2/admin/delegations/users/" }
  ...
  describe "Admin user token operations" do
    before(:each) do
      ...
      @user_id = response.body.first["user_id"]
    ...
    it "retrieves a specific delegation mapping" do
      response = wtoken_header_plain_faraday_json_client_get(admin_token.token,
        delegation_url(@user_id, @delegation_id))
      expect(response.status).to eq(200)
      expect(response.body.keys).to contain_exactly("user_id", "delegation_id")
    end
    ...
    it "create an already existing delegation mapping" do
      ...
      expect(start_with?(response.body["msg"], "ERROR: duplicate key value violates unique constraint")).to eq(true)
    end
    ...
    it "creates and then retrieves a new delegation mapping" do
      ...
      create_resp = wtoken_header_plain_faraday_json_client_post(
        admin_token.token,
        delegation_url(new_user_id, new_delegation_id)
      )
      expect(create_resp.status).to eq(200)
      expect(create_resp.body.keys).to contain_exactly("user_id", "delegation_id")
      ...
      expect(get_resp.body.keys).to contain_exactly("user_id", "delegation_id")
    end
    ...
    it "deletes an existing delegation mapping" do
      ...
      expect(delete_resp.body.keys).to contain_exactly("user_id", "delegation_id")
