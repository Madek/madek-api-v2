...
(defn- get-by-login [table-name login ds]
  (->> (jdbc/execute! ds (-> (sql/select :*) (sql/from table-name) (sql/where [:= :login login]) sql-format))
       (map #(assoc % :type (-> table-name ->PascalCase singular)))
       (map #(clojure.set/rename-keys % {:email :email_address}))
       first))

(defn- get-api-client-by-login [login ds]
  (->> (jdbc/execute! ds (-> (sql/select :*) (sql/from :api_clients) (sql/where [:= :login login]) sql-format))
...
(defn get-entity-by-login-or-email [login-or-email ds]
  (or (get-api-client-by-login login-or-email ds)
      (get-user-by-login-or-email-address login-or-email ds)))
