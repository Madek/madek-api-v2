(ns madek.api.resources.collections.advanced-filter.meta-data
    ...
    (sql/where
     (cons :or
           (into [[:exists
                   (-> (sql/select true)
                       (sql/from :meta_data)
                       (sql/where [:= :meta_data.collection_id :collections.id]
...
(defn filter-by [sqlmap filter-map]
  (let [query (-> sqlmap
                  (sql-filter-by (:meta_data filter-map))
                  (sql-search-through-all (:search filter-map)))]
    ;(info "filter-by" "\nfilter-map:\n" filter-map "\nresult:" query)
    query))
...
;(debug/wrap-with-log-debug #'filter-by)
