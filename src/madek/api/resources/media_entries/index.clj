...
(defn get-preview-list [melist auth-entity tx]
  (let [auth-list (map #(when (true? (media-entry-perms/viewable-by-auth-entity? % auth-entity tx))
                          (get-previews4entry (:id %) tx)) melist)]
...
#_(defn get-cols4me-list [melist auth-entity tx]
  (let [;user-id (:id auth-entity)
...
#_(defn get-md4col-list [collist auth-entity tx]
  (let [user-id (:id auth-entity)
...
(defn build-result-related-data
  ...
  [collection-id auth-entity full-data related-files related-meta-data related-previews data tx]
...
(defn get-index_related_data [{{{collection-id :collection_id 
                                 ...
                                 related-previews :related_previews
                                 } :query} :parameters :as request}]
  ;(try
  (catcher/with-logging {}
    (let [auth-entity (-> request :authenticated-entity)
          data (query-index-resources request)
          tx (:tx request)
          result (build-result-related-data collection-id auth-entity full-data 
                                            ;related-collections
                                            related-files
                                            related-meta-data
                                            related-previews
